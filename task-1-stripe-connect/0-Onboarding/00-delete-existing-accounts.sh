...
ACCOUNTS=$(curl -s https://api.stripe.com/v1/accounts -u "$STRIPE_DEMO_KEY:" -G -d limit=100 \
...
if [ -z "$ACCOUNTS" ]; then
  ...
  for acct in $ACCOUNTS; do
    echo "Deleting $acct..."
    curl -s -X DELETE "https://api.stripe.com/v1/accounts/$acct" -u "$STRIPE_DEMO_KEY:" \
...
curl -s https://api.stripe.com/v1/accounts -u "$STRIPE_DEMO_KEY:" -G -d limit=100 \
