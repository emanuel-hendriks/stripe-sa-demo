...
Use this diagram as a reference image during the presentation. Don't walk through every numbered step -- narrate the four phases verbally:

1. **Discovery** (steps 1-4): User opens map → backend fans out to partners → restaurants rendered as pins.
2. **Menu** (steps 5-8): User taps restaurant → backend fetches menu from partner → app renders categories and items.
3. **Order** (steps 9-13): User confirms → backend POSTs to partner with `Idempotency-Key` + `payment_reference` → partner validates, prices, confirms → backend persists and shows confirmation.
4. **Tracking** (steps 14-25): Direction reverses. Partner pushes webhooks (courier assigned, food ready, en route, delivered) → backend verifies HMAC, updates DB, pushes to user in real time.
...
This mirrors Stripe's Connect onboarding: the platform (Map.co) provisions credentials for each connected partner (Food.co), and both sides authenticate using those credentials. It's the same model as Stripe's OAuth flow for Connect -- the platform provisions the credentials, the connected account uses them, and both sides share a secret for webhook verification. The `api_key` / `webhook_secret` pair here maps directly to Stripe's publishable key / webhook signing secret pair.
...
Frame these as questions you'd raise with the product team -- this shows SA-style thinking, not just spec writing.

**Restaurant closes after menu fetch, before order placement.**
"Do we auto-cancel or let the partner decide? I'd push for the partner to return `422 restaurant_closed` and let Map.co handle the UX -- show the user an error and suggest nearby alternatives."

**Item becomes unavailable between menu fetch and order.**
Food.co returns `422 item_unavailable` with the specific item IDs. Map.co can prompt the user to modify their order rather than failing the entire order.

**Price mismatch between displayed total and partner-confirmed total.**
Food.co validates and prices the order server-side. If the returned `total_amount` differs from what Map.co showed the user (e.g., a menu item price changed between fetch and order), Map.co compares the two. If the delta exceeds a threshold (say 5%), reject the order, re-fetch the menu, and show the user updated pricing. Never charge the user more than what they saw.

**Courier no-show / excessive delay.**
Food.co sends a webhook with status `cancelled` and reason `courier_unavailable`. Map.co handles the refund on their side (since they own payment). "What's our SLA for this? If the courier hasn't picked up after 45 minutes, should Map.co proactively cancel and re-route to another partner?"
...
"What's our SLA for webhook delivery? If Food.co's webhook infra is down for 2 hours, do we poll aggressively or show the user 'status unavailable'?" Map.co should fall back to polling `GET /v1/orders/{id}` at increasing intervals (5s, 15s, 30s) when no webhook has arrived within the expected window.
